...
完整源码及已有提交历史保存在 [GitHub](https://github.com/1003968634-spec/aster)。当前单窗口明信片堆叠版本为 `macos-v0.7.0`；紧凑手写便签与常驻邮递星版本为 `macos-v0.6.0`（`6b148a3`）；信纸与明信片版本为 `macos-v0.5.0`（`4b5e7be`）。纵向便签与开合动画版本为 `macos-v0.4.0`（`688e8e2`）。便签调整前为 `macos-v0.3.0`（`c7c95f2`）。邮票导航调整前保留标签 `macos-before-stamp-navigation`（`949e8ea`）。
